...
, base AS (
    ...
    SELECT 
        ...
        , calc_biz_diff("day", applications.created_at, merchants.created_at) AS biz_days_to_onboard
        , ROW_NUMBER() OVER(
        ...
        , calc_biz_diff("hour", applications.created_at, opportunities.created_at) AS application_to_opportunity_biz_hours
        , DATEDIFF(hour, applications.created_at, applications.modified_at) AS application_created_to_modified_hours
        , calc_biz_diff("hour", applications.created_at, opportunities.modified_at) AS application_created_to_modified_biz_hours
    FROM applications
